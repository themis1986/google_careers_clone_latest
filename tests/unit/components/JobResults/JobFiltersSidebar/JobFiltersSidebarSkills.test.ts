...
describe("JobFiltersSidebarSkills", () => {
  ...
  it("writes user input to store", async () => {
    ...
    expect(userStore.UPDATE_SKILLS_SEARCH_TERM).toHaveBeenLastCalledWith("V");
  });

  it("removes whitespace from ue input", async () => {
    const { userStore } = renderJobFiltersSidebarSkills();
    userStore.skillsSearchTerm = "";
    ...
    expect(userStore.UPDATE_SKILLS_SEARCH_TERM).toHaveBeenLastCalledWith(
      "Vue Developer"
    );
  });
});
